...
def file_is_video(filename):
    return not filename.startswith('.') and os.path.splitext(filename)[1].lower() in ['.avi', '.mp4', '.mkv']
...
class Show:
    ...
    def get_all_episodes(self):
        if not self.episodes:
            ...
            self.add_episodes(iglob(os.path.join(JUSTARRIVED_BASE, self.origname.replace(' ', '.') + ".S*"), as_tuple=True))
...
def write_cache(path, copylist):
    ...
    for showname in sorted(copylist.shows.keys()):
        ...
        if show.last_copied_epnum:
            ...
            s, e = show.startep
        if showname in cache['last_copied_episodes']:
...
class ShowInfo:
    ...
    def __repr__(self):
        ...
        return "%s - last-copied: S%02dE%02d, most-recent: S%02dE%02d (%d files)" % (self.show.name, self.last_copied_epnum[0], self.last_copied_epnum[1], most_recent.season, most_recent.episode, self.count())
...
if __name__ == '__main__':
    ...
    parser.add_argument('dirs', metavar='DIRS', type=str, nargs='*',
                       help='directory to crawl in order to determine the last_copied episodes copied.')
    parser.add_argument('--pretend', "-p", dest='pretend', action='store_const', const=bool, default=False,
                       help="don't actually perform any copies, just show what would be done")
    parser.add_argument('--cachefile', "-c", dest='cache_file',
                       help='cache file to consider when determining copy list. if none is specified, will look for "show_cache" in local directory', default="./show_cache")
    ...
    if args.specified:
        ...
        for specifier in args.specified:
            showname, season, epnum = specifier
            eps.append(Show.get_show(showname).get_episode_from_nums(int(season), int(epnum)))
